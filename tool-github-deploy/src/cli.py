...
class argsData:
    ...
    def initializeArgParse(self):
        ...
        parser.add_argument('-rphi', '--repo_has_issues', help=('repo: argument type Boolean, This argument is used to enable or disable the GitHub repositories "issues" option (default state is True)'))
        parser.add_argument('-rphp', '--repo_has_projects', help=('repo: argument type Boolean, This argument is used to enable or disable the GitHub repositories "Projects" option (default state is True)'))
        parser.add_argument('-rpp', '--repo_private', help=('repo: argument type Boolean, This argument is used to make the repository Private or Public (default state is True)'))
        ...
        parser.add_argument('-rldec', '--disbale_release_error_check', help=('release: argument type Boolean, This argument is used to disable the release error check condition (default state is False)'))
        ...
        parser.add_argument('-digt', '--deploy_ignore_default_all_tags_push', help=('deploy: argument type Boolean, This argument is used to disable the when there is no tag argument and default push all tags to GitHub (default state is False)'))

        # Create Tag Args
        parser.add_argument('-bto', '--bitbucket_tag_operation', help=('bitbucket: argument type Boolean, This argument is used to set the bitbucket tag operation (default state is False)'))
        parser.add_argument('-bid', '--bitbucket_user_id', help=('bitbucket: argument type String, This argument is used to provide the bitbucket user id'))
        parser.add_argument('-bpat', '--bitbucket_personal_access_token', help=('bitbucket: argument type String, This argument is used to provide the bitbucket personal access token'))
        parser.add_argument('-bpn', '--bitbucket_project_name', help=('bitbucket: argument type String, This argument is used to provide the bitbucket project name'))
        parser.add_argument('-btv', '--bitbucket_tag_name', help=('bitbucket: argument type String, This argument is used to provide the bitbucket tag name'))
        parser.add_argument('-bch', '--bitbucket_commit_hash', help=('bitbucket: argument type String, This argument is used to provide the bitbucket commit hash'))
        parser.add_argument('-btd', '--bitbucket_tag_description', help=('bitbucket: argument type String, This argument is used to provide the bitbucket tag description'))

        # Validate GitHub deployment process
        parser.add_argument('-vdo', '--validate_deploy_operation', help=('validate: argument type Boolean, This argument is used to set the Validate GitHub deployment process operation (default state is False)'))
        parser.add_argument('-vmf', '--meta_data_file_path', help=('validate: argument type String, This argument is used to provide the meta data file path'))
        parser.add_argument('-vjsf', '--json_schema_file_path', help=('validate: argument type String, This argument is used to provide the json schema file path'))
        parser.add_argument('-vgrt', '--github_release_tag', help=('validate: argument type String, This argument is used to provide the GitHub release tag version'))
        parser.add_argument('-vgt', '--github_tag', help=('validate: argument type String, This argument is used to provide the GitHub tag version'))
    ...
    def validateMandatoryArgs(self, args):
        ...
        if(args.validate_deploy_operation):
            if(args.meta_data_file_path or args.json_schema_file_path):
                if(not args.meta_data_file_path or not args.json_schema_file_path):
                    print("[!] Invalid number of arguments: [usage: tool-github-deploy.py -vdo=true -vmf=<meta data json file path> -vjsf=<json schema file path>]")
            
            if(not args.github_release_tag and not args.meta_data_file_path and not args.json_schema_file_path and not args.github_tag):
                print("[!] Invalid number of arguments: [usage: tool-github-deploy.py -vdo=true -vmf=<meta data json file path> -vjsf=<json schema file path> or tool-github-deploy.py -vdo=true -vgrt=<validate github release tag version> or tool-github-deploy.py -vdo=true  -vgt=validate github tag version]")

        if(not args.repo_operation and not args.release_operation and not args.deployment and not args.repo_update and not args.bitbucket_tag_operation and not args.validate_deploy_operation):
            exceptionString += ("[!] Should select either one GitHub deploy operation [usage: tool-github-deploy.py -deploy=true ... or tool-github-deploy.py -rlo=true ... or tool-github-deploy.py -rpo=true ... or tool-github-deploy.py -bto=true ... or tool-github-deploy.py -vd  o=true ...]")
    ...
    def validateInputArgFormat(self, args):
        if(args.github_release_tag):
            args.github_release_tag = args.github_release_tag.replace("'", "").replace(" ", "").replace("\"", "")
        
        if(args.github_tag):
            args.github_tag = args.github_tag.replace("'", "").replace(" ", "").replace("\"", "")
        
        if(args.tag_version):
            args.tag_version = args.tag_version.replace("'", "").replace(" ", "").replace("\"", "")
        
        if(args.tag_title):
            args.tag_title=  args.tag_title.replace("'", "").replace(" ", "").replace("\"", "")
    ...
    def argsGet(self):
        ...
        try:
            ...
            self.validateInputArgFormat(args)
